...
// GetFeedDetailWithPage 获取 Feed 详情页数据并返回页面实例
func (f *FeedDetailAction) GetFeedDetailWithPage(ctx context.Context, feedID, xsecToken string) (*FeedDetailResponse, *rod.Page, error) {
	page := f.page.Context(ctx).Timeout(60 * time.Second)
	...
	if result == "" {
		return nil, nil, fmt.Errorf("__INITIAL_STATE__ not found")
	}
	...
	if err := json.Unmarshal([]byte(result), &initialState); err != nil {
		return nil, nil, fmt.Errorf("failed to unmarshal __INITIAL_STATE__: %w", err)
	}

	// 从 noteDetailMap 中获取对应 feedID 的数据
	noteDetail, exists := initialState.Note.NoteDetailMap[feedID]
	if !exists {
		return nil, nil, fmt.Errorf("feed %s not found in noteDetailMap", feedID)
	}
